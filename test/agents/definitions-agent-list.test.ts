...
describe("agent definitions and catalog", () => {
	...
	it("loads the bundled Nicobailon personas as builtin agents", () => {
		const dir = createTestDir();
		const configDir = join(dir, "agent-root");
		mkdirSync(join(configDir, "agents"), { recursive: true });
		process.env.PI_CODING_AGENT_DIR = configDir;
		delete process.env.PI_SUBAGENT_TEST_DISABLE_BUNDLED_AGENTS;
		...
		assert.ok(defs.every((entry) => entry.source === "builtin"));
		assert.ok(defs.every((entry) => entry.mode === "interactive"));
		assert.ok(defs.every((entry) => entry.autoExit === true));
		assert.equal(
			defs.find((entry) => entry.name === "delegate")?.systemPromptMode,
			"append",
		);
		assert.ok(
			defs
				.filter((entry) => entry.name !== "delegate")
				.every((entry) => entry.systemPromptMode === "replace"),
		);
	});

	it("lets global and project definitions override bundled personas", () => {
		const dir = createTestDir();
		const configDir = join(dir, "agent-root");
		const globalAgentsDir = join(configDir, "agents");
		...
		writeFileSync(
			...
			`---\nname: worker\ndescription: Global worker override\nmode: background\n---\n\nGlobal worker.`,
		);
		writeFileSync(
		...
		const defs = getEffectiveAgentDefinitionsForTest(dir);
		assert.equal(defs.find((entry) => entry.name === "worker")?.source, "global");
		assert.equal(defs.find((entry) => entry.name === "scout")?.source, "project");
		assert.equal(defs.find((entry) => entry.name === "reviewer")?.source, "builtin");
	});
